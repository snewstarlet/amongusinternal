...
HRESULT __stdcall D3D_FUNCTION_HOOK(IDXGISwapChain* pThis, UINT SyncInterval, UINT Flags)
{
    ...
    if (CWState::ShowMenu)
    {
        ...
        ImGui::Begin("EkremAbi: AmongUs", &CWState::ShowMenu);
        ...
        ImGui::Spacing();
        ImGui::Spacing();
        ImGui::Text("Author: s.newstarlet");
        ImGui::Spacing();
        ImGui::Spacing();
        ImGui::Spacing();

        if (ImGui::CollapsingHeader("Oyun"))
        {
            if (ImGui::Button("Acil Durum"))
            {
                PlayerControl_CmdReportDeadBody((*PlayerControl__TypeInfo)->static_fields->LocalPlayer, NULL, NULL);
            }
            ImGui::Checkbox("Duvardan Gec", &CWState::NoClip);
            ImGui::Checkbox("Imposterleri goster", &CWState::MarkImposters);
        }

        if (ImGui::CollapsingHeader("Ban Puanciklari"))
        {
            if ((*StatsManager__TypeInfo)->static_fields->Instance == NULL)
            {
                ImGui::Text("Oyuna girin");
            }
            else
            {
                ...
                ImGui::Text(("Ban Puanciklari: " + std::to_string((*StatsManager__TypeInfo)->static_fields->Instance->fields.banPoints)).c_str());
            }
        }

        if (ImGui::CollapsingHeader("Oyuncular") && IsInGame())
        {
            ...
            for (auto player : GetAllPlayers())
            {
                ...
                if (ImGui::Button((std::string("Isinlan") + std::string("##") + name).c_str()))
                {
        ...
        if (ImGui::CollapsingHeader("Kapilar"))
        {
            if (CWState::PinDoors)
            {
                if (ImGui::Button("Hepsini Sec"))
                {
            ...
            ImGui::Checkbox("Kapilari Kapali Tut", &CWState::PinDoors);
        ...
        if (ImGui::CollapsingHeader("Sabotaj1"))
        {
        ...
        if (ImGui::CollapsingHeader("Sabotaj2"))
        {
            for (auto entry : CWConstants::SABOTAGE_SYSTEMS)
            {
        ...
        if (ImGui::CollapsingHeader("GAY MENU"))
        {
            ImGui::Checkbox("Gay ol", &CWState::ColorShift);
        ...
        if ((*PlayerControl__TypeInfo)->static_fields->LocalPlayer != NULL
            ...
            && ImGui::CollapsingHeader("Gorevler"))
        {
            ...
            if (List_1_GameData_TaskInfo__get_Count(tasks, NULL) > 0 
                && ImGui::Button("Butun gorevleri bitir"))
            {
            ...
            for (int i = 0; i < List_1_GameData_TaskInfo__get_Count(tasks, NULL); i++)
            {
                ...
                if (ImGui::Button(("Tamamla##Button" + std::to_string(task->fields.Id)).c_str()) && !task->fields.Complete)
                {
                    PlayerControl_RpcCompleteTask((*PlayerControl__TypeInfo)->static_fields->LocalPlayer, task->fields.Id, NULL);
                }
                ...
                ImGui::TextColored(task->fields.Complete
                    ...
                    , (std::string("Gorev #") + std::to_string(task->fields.Id)).c_str());
            }
        }

        if (ImGui::CollapsingHeader("Degiskenler"))
        {
            ImGui::Text("Gorus alani");
